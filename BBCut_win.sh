...
if [ $5 > 0 ] ; then
	...
	if [ ! -d "./download/${1}/P${5}" ]; then
		mkdir ./download/$1/P$5
		results=`./BBDown ${biliUrl}${1} -p ${5} --work-dir ./download/${1}/P${5} --audio-only`
		downloadFlag=1
	fi
else
	path=./download/${4}
	...
	if [ ! -d "./download/${1}" ]; then
		mkdir ./download/$1
		results=`./BBDown ${1} --work-dir ./download/${4} --audio-only`
		downloadFlag=1
	fi
fi
...
echo "切片时间区间 ${2} - ${3}"
echo "开始处理......"
ffmpeg -ss $2 -i "${path}/${files}" -c copy -t $3 "./output/${4}.mp4" -loglevel quiet
ffmpeg -i "./output/${4}.mp4" -vn -codec copy "./output/${4}".m4a -loglevel quiet
echo "处理完成 输出文件 ./output/${4}.m4a"
